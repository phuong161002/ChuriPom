...
public class PoolManager : SingletonMonobehavior<PoolManager>
{
    ...
    {
        GameObject gameObjectToReuse = null;
        int num = 0;
        do
        {
            gameObjectToReuse = poolDictionary[poolKey].Dequeue();
            poolDictionary[poolKey].Enqueue(gameObjectToReuse);
            num++;
            if(num > poolDictionary[poolKey].Count)
            {
                throw new Exception($"Not enough item {poolKey} in pool");
            }
        }
        while (gameObjectToReuse.activeSelf);
    ...
    public void ResetPool()
    {
        for(int i = 0; i < pool.Length; i++)
        {
            var queue = poolDictionary[pool[i].prefab.GetInstanceID()];
